...
1. **`Pokemon` :**
   - **Attributs Significatifs :** Nous avons choisi d'attribuer à la classe `Pokemon` des caractéristiques telles que les points de vie (`m_pv`), l'attaque (`m_attaque`), et l'élément (`m_element`) pour représenter les principales propriétés d'un Pokémon dans un combat.
...
2. **`Joueur` :**
   - **Responsabilités Clés :** La classe `Joueur` gère la collection de Pokémon du joueur (`m_main`) ainsi que les interactions avec la pioche (`m_pioche`) et la défausse (`m_defausse`).
   - **Gestion de Jeu :** Les méthodes telles que `piocherPokemon`, `ajouterPokemon`, et `retirerPokemon` permettent au joueur d'effectuer des actions pendant le jeu.

3. **`Jeu` :**
   - **Centralisation du Jeu :** La classe `Jeu` agit comme le contrôleur principal du jeu, contenant les instances de joueurs (`m_j1`, `m_j2`), le terrain (`m_terrain`), et les éléments de jeu (`m_listePokemon`).
...
4. **`Tour` :**
   - **Gestion de Tour :** La classe `Tour` encapsule les actions qu'un joueur peut effectuer pendant son tour, telles que piocher une carte, attaquer, et changer de tour.
...
1. **Association `Joueur` - `Pioche`, `Main`, `Defausse` :**
...
2. **Association `Joueur` - `Terrain` :**
   - **Délimitation de l'Espace de Jeu :** Chaque joueur utilise un terrain pour jouer ses cartes Pokémon et gérer les interactions avec l'adversaire.
...
1. **Un tableau d'éléments des pokémons dans `Jeu` plutôt que des classes héritées**
